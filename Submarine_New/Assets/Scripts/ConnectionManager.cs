...
public class ConnectionManager : MonoBehaviourPunCallbacks
{
    ...
    private void TryStartGame()
    {
        ...
        if (PhotonNetwork.CurrentRoom.PlayerCount == PhotonNetwork.CurrentRoom.MaxPlayers)
        {
            ...
            var game = PhotonNetwork.Instantiate("GameMaster", Vector3.zero, Quaternion.identity);
            DontDestroyOnLoad(game);
            SceneManager.LoadScene(this.GameScene + (PhotonNetwork.LocalPlayer.IsMasterClient ? "0" : "1"), LoadSceneMode.Single);
        }
        else
        {
